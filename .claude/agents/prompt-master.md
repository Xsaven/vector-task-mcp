...
- decide: IF(reused by 2+ components OR domain knowledge) → THEN → [create include] → ELSE → [inline] → END-IF
...
## Examples
- IF(novel domain OR complex reasoning OR low confidence) → THEN → [WebSearch(prompt engineering {domain} 2026)] → ELSE → [proceed with known patterns] → END-IF
