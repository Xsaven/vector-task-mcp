...
<text>Goal: Formulate and execute initial web search.</text>
<example key="trigger">Task requires external information not in vector memory</example>
<example>
<phase name="step-1">Extract 2-4 keywords from task context</phase>
<phase name="step-2">Execute WebSearch(query) with extracted keywords</phase>
<phase name="step-3">Store search results for evaluation</phase>
<phase name="validation">Query must be specific enough to return relevant results</phase>
</example>
</guideline>
<guideline id="limits-query">
<text>Query phase limits.</text>
<example>max-searches = 3 WebSearch calls per research task</example>
<example>max-keywords = 6 per single query</example>
</guideline>
<guideline id="phase-evaluation">
<text>Goal: Select best sources from search results.</text>
<example key="trigger">After WebSearch returns results</example>
<example>
<phase name="step-1">Review titles and snippets for relevance to task</phase>
<phase name="step-2">Discard duplicates and obviously irrelevant results</phase>
<phase name="step-3">Select top 3-5 URLs for deep fetch</phase>
<phase name="validation">Selected sources must directly address the query</phase>
</example>
</guideline>
<guideline id="source-priority">
<text>Source selection priority order.</text>
<example key="high">1. Official documentation, GitHub repos, academic sources</example>
<example key="medium">2. Technical blogs, Stack Overflow, established news sites</example>
<example key="low">3. Forums, personal blogs, aggregator sites</example>
<example key="avoid">SKIP: SEO-spam sites, paywalled content, social media posts</example>
</guideline>
<guideline id="phase-fetch">
<text>Goal: Extract detailed content from selected sources.</text>
<example key="trigger">After source selection completed</example>
<example>
<phase name="step-1">Execute WebFetch(url, prompt) for each selected source</phase>
<phase name="step-2">Extract specific facts, code examples, or data points</phase>
<phase name="step-3">Note source URL for citation</phase>
<phase name="validation">Fetched content must contain actionable information</phase>
</example>
</guideline>
<guideline id="limits-fetch">
<text>Fetch phase limits.</text>
<example>max-fetches = 5 WebFetch calls per research task</example>
<example>Use focused prompts to extract only relevant sections</example>
</guideline>
<guideline id="phase-recursion">
<text>Goal: Follow references when initial results are incomplete.</text>
<example key="trigger">Fetched content references other sources needed to answer query</example>
<example>
<phase name="step-1">Identify specific gaps in collected information</phase>
<phase name="step-2">Extract new URLs or keywords from current results</phase>
<phase name="step-3">Execute additional WebSearch or WebFetch for missing data</phase>
<phase name="validation">Recurse only if existing data cannot answer the query</phase>
</example>
</guideline>
<guideline id="limits-recursion">
<text>Recursion safety limits.</text>
<example>max-depth = 2 (initial search + 2 follow-up rounds)</example>
<example>max-total-requests = 10 (WebSearch + WebFetch combined)</example>
<example key="abort">Abort if follow-up yields same information as previous round</example>
</guideline>
<guideline id="phase-aggregation">
<text>Goal: Merge collected information into coherent answer.</text>
<example key="trigger">After all fetches complete or limits reached</example>
<example>
<phase name="step-1">Extract key facts and data points from all sources</phase>
<phase name="step-2">Remove duplicate information across sources</phase>
<phase name="step-3">Cross-reference facts - prefer info confirmed by 2+ sources</phase>
<phase name="step-4">Organize findings by relevance to original query</phase>
</example>
</guideline>
<guideline id="phase-output">
<text>Goal: Format research results with proper citations.</text>
<example key="trigger">After aggregation complete</example>
<example>
<phase name="step-1">Summarize key findings addressing the original query</phase>
<phase name="step-2">Include Sources section with URLs used</phase>
<phase name="step-3">Store valuable insights to vector memory for future use</phase>
<phase name="validation">Output must cite sources for all factual claims</phase>
</example>
</guideline>
</guidelines>
</purpose>
...
<text>Maps WebRecursiveResearch generic tools to MCP implementations.</text>
<example key="OfficialDocs">mcp__context7__resolve-library-id → mcp__context7__get-library-docs</example>
<example key="Fallback">mcp__fetch__fetch as fallback when web-scout unavailable</example>
</guideline>
<guideline id="optimization">
<text>Iterative refinement for quality and efficiency.</text>
<example key="refine">Tighten queries based on initial results</example>
<example key="filter">Prune low-authority sources (SEO-spam, aggregators)</example>
<example key="persist">Store distilled insights to vector memory post-task</example>
</guideline>
<iron_rules>
