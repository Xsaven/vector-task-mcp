...
MUST get explicit user approval BEFORE creating any task. EXCEPTION: If $HAS_Y_FLAG is true, auto-approve task creation (skip user confirmation prompt).
...
- on_violation: Present task specification and wait for explicit YES/APPROVE/CONFIRM (unless $HAS_Y_FLAG is true)
...
MUST perform comprehensive research BEFORE formulating task: existing tasks, vector memory, codebase (if code-related), documentation.
...
- on_violation: STOP. Execute ALL research steps (existing tasks, memory, codebase exploration) before proceeding to analysis.
...
ALL research steps (existing tasks, vector memory, codebase, documentation) MUST be delegated to specialized agents. NEVER execute research directly.
...
- on_violation: STOP. Delegate to: vector-master (tasks/memory), explore (codebase), documentation-master (docs). Never use direct MCP/Glob/Grep calls for research.
...
- parse: TASK → [STORE-GET($TASK_DESCRIPTION)] → END-TASK
...
- decompose-check: IF(estimate > 8 hours) → THEN → [STRONGLY RECOMMEND: Run /task:decompose STORE-GET($CREATED_TASK_ID) to break down this large task] → END-IF
