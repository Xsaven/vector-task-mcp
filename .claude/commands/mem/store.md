...
- analyze: IF(similar memories found with similarity > 0.8) → THEN → [WARN: Similar memory exists (ID: {id}, similarity: {score}) → Show: "{content_preview}" → Ask: "Continue storing? (yes/no/update existing)"] → ELSE → [STORE-AS($DUPLICATES = 'none')] → END-IF

# Workflow step3
STEP 3 - Analyze Content and Suggest Category/Tags
## Examples
- detect-category: IF(STORE-GET($CATEGORY) is empty) → THEN → [Analyze content domain and type → Suggest category based on content analysis] → END-IF
- detect-tags: IF(STORE-GET($TAGS) is empty) → THEN → [Extract key topics, technologies, concepts → Suggest 3-5 relevant tags] → END-IF
...
- display-4: Tags: {tags}
- display-5: IF(STORE-GET($DUPLICATES) !== none) → THEN → [WARNING: Similar memories exist!] → END-IF
