...
- `18`: IF(STORE-GET($NEEDS_RESEARCH)) →
  ...
  3.3: IF context7 insufficient → [DELEGATE] @web-research-master: 'Research: {task.title}. Find: implementation patterns, best practices.'
...
- `27`: IF(STORE-GET($DOCS_INDEX) found) →
  [DELEGATE] @explore: 'Read docs: {doc.paths}. Return full content.' → STORE-AS($DOCS_CONTENT)
  DOCS_CONTENT = COMPLETE specification. Pass to ALL agents. Documentation > task.content.
→ END-IF
- `28`: [DELEGATE] @explore: 'Find SIMILAR/ANALOGOUS implementations in codebase for: {task.title}. Search: analogous class names, method patterns, trait usage, helper utilities, base classes. Return: {similar_files, approach, conventions, reusable_code}. Exclude: .brain/' → STORE-AS($EXISTING_PATTERNS)
...
- `40`: 6.1 PARALLEL: Independent tasks → multiple [DELEGATE] @{agent}: '{subtask + security + validation instructions}' in ONE message
...
- `3`: After implementation → [DELEGATE] @explore: 'Run tests. Detect framework (jest, pytest, phpunit, pest, cargo test, go test). Report pass/fail.'
- `4`: IF(all tests pass) →
