...
- `1`: IF(search_mode === "discovery") →
  Task(@web-research-master, 'INPUT(STORE-GET($CURRENT_YEAR) && STORE-GET($CACHED_PATTERNS) && STORE-GET($CACHE_VALID) && STORE-GET($CACHE_AGE))', TASK →
...
- `2`: Task(@web-research-master, 'INPUT(STORE-GET($PROJECT_STACK.TECHNOLOGIES) && STORE-GET($CURRENT_YEAR) && STORE-GET($SEARCH_FILTER) && STORE-GET($SEARCH_MODE))', TASK →
...
- `3`: Task(@agent-master, 'INPUT(STORE-GET($PROJECT_AGENTS) && STORE-GET($SYSTEM_AGENTS) && STORE-GET($PROJECT_STACK) && STORE-GET($INDUSTRY_PATTERNS) && STORE-GET($TECH_PATTERNS) && STORE-GET($SEARCH_FILTER))', TASK →
...
- `9`: FOREACH(batch in $AGENT_BATCHES (PARALLEL)) →
  Task(@agent-master, 'INPUT(batch_agents = [{name, purpose, capabilities, confidence, variation: "Master"}, ...] && STORE-GET($INDUSTRY_PATTERNS) && STORE-GET($TECH_PATTERNS) && STORE-GET($PROJECT_AGENTS))', TASK →
...
# Phase6 compile
GOAL(Compile all agents to .opencode/agent/)
- `1`: Bash(brain compile) → [Compiles .brain/node/Agents/*.php to .opencode/agent/] → END-Bash
- `2`: VERIFY-SUCCESS(CHECK(.opencode/agent/ for new agent files) Compilation `completed` without errors)
...
- `1`: Created: .brain/node/Agents/ | Compiled: .opencode/agent/
- `2`: Ready via: [DELEGATE] @{name}: '...'
...
- Gate 12: .opencode/agent/ populated with project agents
